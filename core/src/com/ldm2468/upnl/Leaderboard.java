...
public class Leaderboard {
    public static void post(String name, long score, boolean hell) {
        try {
            Net.HttpRequest request = new Net.HttpRequest("POST");
            Map<String, String> params = new HashMap<String, String>();
            params.put("name", name);
            params.put("score", "" + score);
            byte[] md5 = MessageDigest.getInstance("MD5").digest((name + score + "$alt" + (hell ? "2" : "")).getBytes());
            ...
            request.setUrl("http://ldm2468.com/upnl-leaderboard/post" + (hell ? "2" : "") + ".php");
